...
import { ArrowLeft, Cpu, HeartHandshake, Shield, Smartphone, Sparkles } from 'lucide-react';
...
const pillars = [
  {
    ...
    description: "Une interface unique pour suivre tes finances, tes études, tes objectifs et tes routines sans changer d'écran.",
    icon: Cpu,
    tone: toneClassNames.info,
  },
  {
    title: 'Mobile d’abord',
    description: 'Pensée pour un usage rapide, lisible et tactile sur téléphone, avec une navigation fluide et des cartes compactes.',
    icon: Smartphone,
    tone: toneClassNames.warning,
  },
  {
    title: 'Confidentialité locale',
    description: 'Tes données restent gérées localement dans l’application afin de garder un contrôle simple et direct sur ton espace.',
    icon: Shield,
    ...
    <div className="space-y-8 pb-28 animate-in fade-in duration-500">
      <div className="relative border-b border-[color:var(--border)] pb-8 pt-1">
        ...
        <div className="mx-auto flex max-w-3xl flex-col items-center px-10 text-center md:px-0">
          <div className={cx('mb-3 flex h-12 w-12 items-center justify-center rounded-2xl', toneClassNames.warning.shell, toneClassNames.warning.icon)}>
            <Sparkles size={18} />
          </div>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-[color:var(--text-secondary)] md:text-base">
            Un espace personnel pour organiser tes journées, clarifier tes priorités et suivre tes progrès avec une interface cohérente en light et dark mode.
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {pillars.map((pillar) => (
            <div key={pillar.title} className={cx(uiRecipes.card, 'rounded-[1.5rem] p-5')}>
              <div className={cx('flex h-11 w-11 items-center justify-center rounded-2xl', pillar.tone.shell, pillar.tone.icon)}>
                <pillar.icon size={18} />
              </div>
      ...
      <div className="grid gap-4 md:grid-cols-[minmax(0,1.35fr)_minmax(0,0.9fr)]">
        <div className={cx(uiRecipes.sectionCard, 'rounded-[2rem] p-6 md:p-7')}>
          <div className="flex items-center gap-3">
            <div className={cx('flex h-11 w-11 items-center justify-center rounded-2xl', toneClassNames.info.shell, toneClassNames.info.icon)}>
          ...
          <p className="mt-5 text-sm leading-relaxed text-[color:var(--text-secondary)]">
            L’objectif de l’app est de rassembler dans un seul environnement les fonctions clés du quotidien : suivi budgétaire, objectifs, discipline, études et routines. Le tout avec une expérience mobile propre, rapide et agréable.
          </p>
        </div>
        ...
        <div className={cx(uiRecipes.card, 'rounded-[2rem] p-6 md:p-7')}>
          ...
          <div className="mt-5 space-y-4">
            <div>
              <p className="text-[9px] font-black uppercase tracking-[0.22em] text-[color:var(--text-muted)]">Version</p>
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-[color:var(--text-secondary)]">Application locale et themable</p>
            </div>
            <div>
              <p className="text-[9px] font-black uppercase tracking-[0.22em] text-[color:var(--text-muted)]">Navigation</p>
              <p className="mt-1 text-sm font-semibold text-[color:var(--text-secondary)]">Accessible depuis les réglages</p>
            </div>
          </div>
        </div>
      </div>
