...
export const QuotaAnalysis: React.FC<QuotaAnalysisProps> = ({
  ...
  return (
    <ModalShell
      ...
      onClose={onClose}
      title="Analyse Quota"
      subtitle="Lecture de quota journaliere"
    ...
    >
      <div className="text-center">
        <div className="relative mx-auto mb-4 flex h-40 w-40 items-center justify-center rounded-full border-4 border-slate-800 sm:h-48 sm:w-48">
          <svg className="absolute inset-0 h-full w-full -rotate-90">
            <circle cx="50%" cy="50%" r="46%" className="fill-none stroke-slate-800" strokeWidth="8" />
            <circle
              cx="50%"
              cy="50%"
              r="46%"
          ...
          <div>
            ...
            <span className="mt-1 block text-[9px] font-black uppercase tracking-widest text-slate-500">Utilise</span>
          </div>
        </div>

        <div className="flex items-center justify-center gap-6">
          <div>
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-500">Depense</p>
            <p className="text-xl font-black text-white">{safeTodaySpent.toLocaleString()} DH</p>
          </div>
      ...
      <div className="rounded-[1.5rem] border border-white/5 bg-slate-900/40 p-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <label className="flex min-w-0 flex-1 items-center gap-3 rounded-[1.25rem] border border-white/10 bg-[#020617] px-4 py-3.5">
            <span className="text-[10px] font-black uppercase tracking-[0.24em] text-slate-500">Quota</span>
            <input
              type="number"
              min="0"
              inputMode="numeric"
            ...
            />
            <span className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">DH</span>
          </label>
        ...
        {hasCustomQuota ? (
          <button
            ...
            disabled={isSavingQuota}
            className="mt-3 text-[10px] font-black uppercase tracking-[0.22em] text-slate-500 transition-all hover:text-slate-300 disabled:opacity-40"
          >
      ...
      {isOver ? (
        <div className="flex items-start gap-3 rounded-[1.5rem] border border-rose-500/20 bg-rose-500/10 p-4">
          ...
          <div>
            <h4 className="mb-1 text-[10px] font-black uppercase tracking-widest text-rose-500">Depassement detecte</h4>
            <p className="text-[11px] text-slate-300">
              Vous avez depasse votre quota de <span className="font-bold text-white">{Math.abs(remainingToday).toLocaleString()} DH</span>. Cela sera deduit du budget des {safeDaysUntilReset} prochains jours.
            </p>
          </div>
        </div>
      ) : null}
      ...
      <div className="flex flex-col items-center justify-center gap-2 rounded-[1.5rem] border border-white/5 bg-slate-900 p-4 text-center">
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Jours restants</span>
